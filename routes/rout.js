...
function ShareName() {
    pool.query("SELECT name FROM pc_tu WHERE name LIKE '%STUD%' AND name NOT LIKE '%TEACH' ORDER BY name ASC;", function (err, data) {
        if (err) return console.log(err);
...
router.post("/*/Inventory", urlencodedParser,function (request, response) {
    ...
    if(/10.10.99.[0-9]{1,3}/g.test(ip)){
        ...
        if(request.body.upvote == undefined){
            pool.query("SELECT * FROM pc_tu WHERE name NOT LIKE '%TEACH%' AND name NOT LIKE '%STUD%' ORDER BY name ASC;", function (err, data) {
                if (err) return console.log(err);
                Data_length = data.length;
                inv_data = data;
                ExcelCreate();
...
router.post("/*/Inventory/Teach", urlencodedParser,function (request, response) {
    ip = request.socket.remoteAddress;
    if(10.10.99.[0-9]{1,3}/g.test(ip)){
        if (!request.body) return response.sendStatus(400);
        ShareName();
            pool.query("SELECT * FROM pc_tu WHERE name LIKE '%TEACH%' ORDER BY name ASC;", function (err, data) {
                if (err) return console.log(err);
                Data_length = data.length;
                inv_data = data;
                ExcelCreate();
...
router.post("/*/Net/Teach", urlencodedParser,function (request, response) {
    ip = request.socket.remoteAddress;
    if(/10.10.99.[0-9]{1,3}/g.test(ip)){
        if (!request.body) return response.sendStatus(400);
        ShareName();
...
router.post("/update", jsonParser, function (request, response) {
    ...
    if(/10.10.99.[0-9]{1,3}/g.test(ip)){
        ...
        pool.query("SELECT id FROM pc_tu WHERE mac LIKE '" + `${request.body.Old}` + "%'", function (err, data) {
            if (err) return console.log(err);
            console.log(data);
            for (let i = 0; i < data.length; i++) {
                id_old_sn = data[i].id;
